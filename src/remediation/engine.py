...
Supports both Finding dataclass and plain dict inputs for flexibility.
"""
...
class RemediationResult:
    finding: Union[Finding, dict]
    attempts: List[PatchAttempt] = field(default_factory=list)
    success: bool = False
    final_patch: Optional[str] = None


def _extract_finding_attrs(finding: Union[Finding, dict]) -> dict:
    """Extract attributes from either a Finding dataclass or a plain dict."""
...
def assemble_context(finding: Union[Finding, dict], file_path: Path, context_lines: int = 10) -> str:
    """Extract surrounding code context around the vulnerable line."""
    attrs = _extract_finding_attrs(finding)
    try:
        lines = file_path.read_text(encoding="utf-8", errors="ignore").split("\n")
        start = max(0, attrs["line_number"] - context_lines - 1)
        end = min(len(lines), attrs["line_number"] + context_lines)
        return "\n".join(lines[start:end])
    except Exception:
        return attrs["code_snippet"]
...
def generate_patch(
    ...
    Works with both Finding dataclass and plain dict.
    Supports both sync and async llm_call.
    """
    attrs = _extract_finding_attrs(finding)
    file_context = assemble_context(finding, file_path)
    sanitized_snippet = sanitize_code(attrs["code_snippet"])
    sanitized_context = sanitize_code(file_context)

    prompt = PATCH_GENERATOR_PROMPT.format(
        vuln_type=attrs["vuln_type"],
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(llm_call):
            response = asyncio.get_event_loop().run_until_complete(llm_call(prompt))
        else:
...
async def generate_patch_async(
    ...
    """Async version of generate_patch."""
    attrs = _extract_finding_attrs(finding)
    file_context = assemble_context(finding, file_path)
    sanitized_snippet = sanitize_code(attrs["code_snippet"])
    sanitized_context = sanitize_code(file_context)
...
async def remediate_finding(
    ...
    for attempt_num in range(1, MAX_REMEDIATION_ATTEMPTS + 1):
        logger.info(
            "remediation_attempt",
            vuln_id=attrs["vuln_type"],
            file=str(file_path),
            attempt=attempt_num,
        )

        patch = await generate_patch_async(finding, file_path, llm_call)
